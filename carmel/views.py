...
import numpy as np
from account.models import Student,LastFace
...
def teachers(request):
    a=Faculty.objects.filter(role='Teacher')
    b=Faculty.objects.filter(role='H.O.D')
    c=Faculty.objects.filter(role__in=['Principal','Vice-Principal'])
    dict={
        'teacher': a,
        'hod':b,
        'admin':c
    ...
    return render(request,'teachers.html',dict)

def departments(request):

    a=Department.objects.exclude(name='Administration')
    return render(request,'department.html',{'department':a})
